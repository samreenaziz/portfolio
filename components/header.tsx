                ...
                <ul className="flex w-[22rem] justify-center items-center flex-wrap gap-y-1 px-0 sm:px-5 pt-9 sm:pt-0 text-[0.9rem] sm:text-[1rem] font-medium text-grey-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
                    {
                        links.map(link =>(
                            ...
                            animate={{y:0, opacity:1}}>
                                <Link className={clsx ("flex w-full items-center justify-center px-3 py-3  cursor-pointer hover:bg-gray-100 hover:text-blue-900 rounded-none hover:rounded-full", {"text-blue-900 bg-gray-100 !rounded-full": activeSection === link.name})} href={link.hash} onClick={() => setActiveSection(link.name)}>
                                    {link.name}
                                </Link>
                            </motion.li>
                        ))
